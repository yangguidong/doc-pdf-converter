...
async function renderStack(){if(!stackItem)return;stackB.innerHTML='';const imgs=(stackItem.imgs&&stackItem.imgs.length)?stackItem.imgs:[stackItem.id];stackCnt.textContent=imgs.length+' 张';
  for(let j=0;j<imgs.length;j++){const key=imgs[j];
    ...
    const wrap=document.createElement('div');wrap.className='stack-img-wrap';const blob=await dbGet(key);let imgUrl;if(blob){imgUrl=URL.createObjectURL(blob)}else if(key.startsWith('v_')){imgUrl='uploads/'+key+'.mp4'}else{imgUrl='uploads/'+key+'.jpg'}if(imgUrl&&!key.startsWith('v_')){const img=document.createElement('img');img.src=imgUrl;img.onerror=function(){this.src=this.src.replace('.jpg','.jpeg');this.onerror=function(){this.src=this.src.replace('.jpeg','.png')}};wrap.appendChild(img)}else if(imgUrl){const vid=document.createElement('video');vid.src=imgUrl;vid.controls=true;vid.playsInline=true;vid.setAttribute('playsinline','');vid.setAttribute('webkit-playsinline','');vid.setAttribute('allowfullscreen','');vid.preload='auto';vid.style.cssText='max-width:60%;border-radius:2px;box-shadow:0 4px 30px rgba(0,0,0,.5);background:#000';wrap.appendChild(vid)}else{wrap.innerHTML='<div style="display:inline-block;aspect-ratio:4/3;min-width:200px;background:var(--s);color:var(--td);font-size:.7rem;padding:40px;">No Media</div>'}
